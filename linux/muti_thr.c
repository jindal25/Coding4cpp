...
std::thread thr_a, thr_b, thr_c;

int main()
{
std::thread main_thr;

main_thr = std::this_thread::get_id ();
printf("Main thread = %d\n", main_thr); 

if (thr_b = std::thread(sub_b)
        fprintf(stderr,"Can't create thr_b\n"), exit(1);

if (thr_a = std::thread(sub_a))
        fprintf(stderr,"Can't create thr_a\n"), exit(1);

if (thr_c = std::thread(sub_c))
...
void *sub_a(void *arg)
{
std::thread thr_b = (thread_t) arg;
std::thread thr_d;
int i;

printf("A: In thread A...\n"); 

if (thr_d = std::thread(NULL, 0, sub_d, (void *)thr_b, THR_NEW_LWP, &thr_d))
